...
import {
    ...
    resumeFunc,
    playNextFunc,
} from "../player-logic/AudioController";
...
export class Music extends Component {
        ...
        if (playbackStatus.didJustFinish) {
            ...
            if (nextAudioIndex >= this.context.totalAudioCount) {
                ...
                return await storeAudioForNextOpening(
                    this.context.audioFiles[0],
                    0
                );
            }

            const audio = this.context.audioFiles[nextAudioIndex];
            const status = await playNextFunc(
                this.context.playbackObject,
                audio.uri
            );
    ...
    handleNext = async () => {
        ...
        try {
            ...
            const status = isLoaded
                ? await playNextFunc(playback, nextAudio.uri)
                : await playFunc(playback, nextAudio.uri);
    ...
    handlePrevious = async () => {
        ...
        try {
            ...
            const nextAudioIndex = isFirstAudio
                ? totalAudioCount - 1
                : currentAudioIndex - 1;
            const nextAudio = audioFiles[nextAudioIndex];
                            ...
                            /> */}
                                ...
                                <View className="self-center w-full border-t-2 border-solid border-secondary">
                                    <View className="w-[96%] self-center">
                                        <MicroPlayer
                                            isFirstLaunch={() => {
                                                this.context.firstLaunch;
                                            }}
                                            title={
                                                this.context.currentAudio
                                                    .filename ||
                                                "Hey, wanna listen "
                                            }
                                            duration={
